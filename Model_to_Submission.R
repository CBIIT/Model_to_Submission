  ...
  for (prop in props){
    if (prop %in% unique(TaVS$`Value Set Name`)){
      ...
      if (start_pos==1){
        ...
        suppressWarnings(dataValidation(wb = wb, sheet = node, cols= col_pos,rows = 2:100000, type="list",value = paste("'Terms and Value Sets'!$C$",start_pos,":$C$",stop_pos+1,sep="")))
      }else if (!is.na(stop_pos)){
        col_pos=grep(pattern = TRUE, x = (colnames(metadata) %in% prop))
        suppressWarnings(dataValidation(wb = wb, sheet = node, cols= col_pos,rows = 2:100000, type="list",value = paste("'Terms and Value Sets'!$C$",start_pos,":$C$",stop_pos,sep="")))
      }else{
        col_pos=grep(pattern = TRUE, x = (colnames(metadata) %in% prop))
        suppressWarnings(dataValidation(wb = wb, sheet = node, cols= col_pos,rows = 2:100000, type="list",value = paste("'Terms and Value Sets'!$C$",start_pos,":$C$",dim(TaVS)[1],sep="")))
      }
    }
  }
  addStyle(wb = wb, sheet = node, style = text_format,rows = 2:100000, cols = 1:length(props), gridExpand = TRUE)
}
